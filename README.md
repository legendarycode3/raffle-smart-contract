...
## Architecture
The Raffle system follows a decentralized, event-driven architecture where the `Raffle` smart contract manages participant entries, raffle state, winner selection, and prize distribution, while Chainlink services provide automation
...
* ****Multi-Network Support:**** Deployable on localhost (Anvil) and Sepolia Eth testnet. Easily deployable across different networks, using automated scripts. 
...
* ****Solidity(^0.8.19)**** : The programming language for writing the Smart contracts.
...
4. **Configure Makefile:**
  * Change account name in Makefile to the name of your desired encrypted key.
    * change "--account defaultKey" to "--account <YOUR_ENCRYPTED_KEY_NAME>"  (if tryintg to deploy on `sepolia testnet network` rather than `anvil`)
...
* **End-to-End Flow Tests** Simulate the complete raffle lifecycle from player entry through winner selection and raffle reset.
...
Or Interact with Raffle contract deployed on sepolia testnet: 
 > Directly (from [Sepolia Etherscan](https://sepolia.etherscan.io/) 
...
* ****`HelperConfig.sol`:**** Network configuration management. The HelperConfig script (typically named HelperConfig.s.sol) is the utility script used  to manage and provide deployment parameters (E.G such as RPC URLs, Chainlink oracle addresses, and initial fees) based on the specific blockchain network you are targeting (e.g. like  Sepolia,  local Anvil node).
...
* ****Efficient State Management****: The contract updates only the required state variables during each raffle cycle, avoiding unnecessary storage writes and reducing execution costs.
...
* ****Professional Audit Recommendation****: Although the contract includes several security best practices, a professional smart contract audit is recommended before deploying to mainnet or handling significant funds.
...
* Immutable configuration values such as entrance fee, interval, gas lane, subscription ID, and callback gas limit cannot be changed after deployment.
